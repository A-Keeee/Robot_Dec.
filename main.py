...
#        car.send_speed(0, 0, 0)  # 停止移动
    ...
    if y_speed != 0 and z_speed !=0:
    # 控制小车的运动
        car.chassis_control(0.5*int(x_speed), 0.5*int(y_speed), 0.5*int(z_speed))#y向右为正 z逆时针为正
    print("y_speed: {}, z_speed: {}".format(y_speed, z_speed))
        ...
        if flag == b'$OK!':
            ...
            if flag[0:8] == "DISTANCE" :
                ...
                if distance < 20 and mode == 0 and counter == color_num:#具体距离待定
                    ...
                    turn_180(direction)
                #未到目标点 转向巡线
                elif distance < 20 and mode == 0 and counter < color_num and color_num !=3:#具体距离待定
                    counter = turn_90(direction,counter)
                #未到目标点 转向巡线（放红色）
                elif color_num == 3 and distance < 20 and mode == 0 and color_num ==3:#具体距离待定
                    counter = turn_90(direction,counter)
                    counter +=2
                #掉头完毕到达目标点 停下来进行抓/放
