...
pid_turn = PID(p=1.3, i=0.1, d=0.015)           # 用于转向（旋转角度）的PID
...
#    print(move_x)
    ...
    if valid_points:
        ...
        turn_right = False
        tolerance_turn = 200
        if valid_points[0] < max_pt-tolerance_turn :
            turn_left = True
        if valid_points[0] > min_pt+tolerance_turn :
            turn_right = True
        tolerance = 20  # 设置容差值，根据实际情况调整
...
#    move_turn_err = desired_move_turn - move_turn
    ...
    y_speed = -pid_x.get_pid(move_x_err, 2.5)  # 基于 move_x_err 控制 y 速度（左右偏移的调整）
    z_speed = pid_turn.get_pid(move_turn_err, 2)  # 基于 move_turn_err 控制 z 速度（旋转角度的调整）
    ...
    x_speed = 500 - abs(z_speed)*0.5  # 固定的前进速度，也可以根据需要调整
